...
&最长正面序列的界，L_j表示最长序列长度是j的事件,最长序列长度为L，期望：\\
&E[L] = \sum_{j=0}^{n}{jP_r\{L_j\}} \\
&\qquad = \sum_{j=0}^{2[\lg{n}]-1}{jP_r\{L_j\}}+\sum_{j=2[\lg{n}]}^{n}{jP_r\{L_j\}}\\
&\qquad <\sum_{j=0}^{2[\lg{n}]-1}{2[\lg{n}]P_r\{L_j\}}+\sum_{j=2[\lg{n}]}^{n}{nP_r\{L_j\}}\\
&\qquad = 2[\lg{n}]\sum_{j=0}^{2[\lg{n}]-1}{P_r\{L_j\}}+n\sum_{j=2[\lg{n}]}^{n}{P_r\{L_j\}}\\
&\qquad < 2[\lg{n}]+n(1/n) = \Omicron(\lg{n})\\
\end{aligned}
$$